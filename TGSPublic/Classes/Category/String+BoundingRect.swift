...
public extension String {
   ...
   func sizeOfText(withMaxSize maxSize: CGSize, font: UIFont?) -> CGSize {
       ...
       let size = self.boundingRect(with: maxSize, options: .usesLineFragmentOrigin, attributes: dict, context: nil).size
       return size
   }
   
   //类方法
    static func size(withText text: String?, maxSize: CGSize, font: UIFont?) -> CGSize {
       return text?.sizeOfText(withMaxSize: maxSize, font: font) ?? CGSize.zero
   }
    ...
    /// 给定最大宽高计算宽度和高度，传入字体、行距、对齐方式（便捷调用）
    func textSizeForLabel(label:UILabel, width: CGFloat, height: CGFloat, font: UIFont, lineSpacing: CGFloat = 5, alignment: NSTextAlignment = .left) -> CGSize {
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = lineSpacing
//        paragraphStyle.lin
        paragraphStyle.alignment = alignment
        ...
        let att = NSAttributedString(string: self, attributes: attributes)
